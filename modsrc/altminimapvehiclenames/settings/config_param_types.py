# Param types for the config system.
# (Identisches Framework wie im DistanceMarker-Mod verwendet.)

import logging

from altminimapvehiclenames.settings import toJson, toColorTuple, toBool, clamp, toPositiveFloat
...
class Param(object):
    ...
    def jsonValue(self, jsonValue):
        try:
            ...
            logger.warn('Error occurred while saving parameter %s with jsonValue %s, fallback to previous valid value.', self.tokenName, jsonValue, exc_info=e)
    ...
    def msaValue(self, msaValue):
        try:
            ...
            logger.warn('Error occurred while saving parameter %s with msaValue %s, fallback to previous valid value.', self.tokenName, msaValue, exc_info=e)
...
class BooleanParam(Param):
    ...
    def renderParam(self, header, body=None, note=None, attention=None):
        return {'type': 'CheckBox',
           ...
           'tooltip': (createTooltip(header='%s (%s: %s)' % (header, Tr.DEFAULT_VALUE, Tr.CHECKED if self.defaultValue else Tr.UNCHECKED), body=body, note=note, attention=attention))}
...
class StepperParam(NumberParam):
    ...
    def __init__(self, path, castFunction, minValue, step, maxValue, defaultValue, disabledValue=None):
        super(StepperParam, self).__init__(path, castFunction, minValue, step, maxValue, defaultValue, disabledValue)

    def renderParam(self, header, body=None, note=None, attention=None):
        return {'type': 'NumericStepper',
           'text': header,
           'varName': (self.tokenName),
           'value': (self.defaultMsaValue),
           ...
           'tooltip': (createTooltip(header='%s (%s: %s)' % (header, Tr.DEFAULT_VALUE, self.defaultMsaValue), body=body, note=note, attention=attention))}
...
class Option(object):
    ...
    def __init__(self, value, msaValue, displayNameToken):
        self.value = value
        self.msaValue = msaValue
        self.displayNameToken = displayNameToken
...
class OptionsParam(Param):
    ...
    def renderParam(self, header, body=None, note=None, attention=None):
        defaultOption = self.getOptionByValue(self.defaultValue)
        return {'type': 'Dropdown',
           'text': header,
           'varName': (self.tokenName),
           'value': (self.defaultMsaValue),
           'options': [{'label': (getTranslation(option.displayNameToken))} for option in self.options], 'tooltip': (createTooltip(header='%s (%s: %s)' % (header, Tr.DEFAULT_VALUE, getTranslation(defaultOption.displayNameToken)), body=body, note=note, attention=attention)),
           'width': 200}
