...
3. **Eigene Zugmitglieder (Squad/Platoon)** sind **IMMER** sichtbar und zeigen
   - **ohne Alt** den **Spielernamen**
   - **mit Alt** den **Fahrzeugnamen**
4. Die beiden Zug-Dropdowns gelten **nur fuer den eigenen Zug**. Andere
   verbuendete Zuege bekommen weder eigene Spielernamen noch den eigenen
   Zug-Namensmodus.
5. Erkannte gegnerische Zuege koennen mit einem Sternchen oder der
   hochgestellten Zugnummer aus der Teamliste markiert werden. Wenn die
   Feind-Bezeichnung gerade ausgeblendet ist, kann die Markierung allein
   sichtbar bleiben.
6. **Fahrzeug-Symbole (Icons)** bleiben unberuehrt (Vanilla-Verhalten).
...
- Pro Fahrzeug kennt Python das Team via `VehicleEntry.isEnemy()`
  (`entries.py`).
- `ArenaDataProvider.isSquadMan(vehicleID)` filtert auf eigenes Team und die
  `prebattleID` des eigenen Fahrzeugs; das allgemeine `vInfo.isSquadMan()`
  allein wuerde auch fremde Zuege markieren.
- `VehicleArenaInfoVO.squadIndex` ist die vom Client vergebene Zugnummer pro
  Team und entspricht der Nummerierung der Teamliste.
...
| Methode | Zweck |
|---------|-------|
| `__handleShowExtendedInfo` | Original ausfuehren, danach team-abhaengige Namen anwenden. Nur der eigene Zug nutzt den squad-names-Modus; andere Verbundete nutzen ally-names, Gegner enemy-names. |
| `_setVehicleInfo` | Bei jedem (auch neu erscheinenden) Fahrzeug Team, eigenen Zug, Zugnummer und Anzeigeinhalt erfassen; eigene Zugnamen sowie gegnerische Marker werden sofort nachgesetzt. |
...
- Die Zugnummer ist nur gueltig, wenn der Client den Zug erkannt und
  `squadIndex > 0` gesetzt hat. Die Nummerndarstellung nutzt Unicode-
  Hochziffern, weil die vorhandene `VehicleMinimapEntry`-Textfield-API kein
  verifiziertes `<sup>`-Tag anbietet.
